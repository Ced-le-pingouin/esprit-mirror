...
class Spreadsheet_Excel_Reader {
	...
	*/
	...
	function readCSV($sFileName) {
		// CSV
		if (!is_readable($sFileName))
			die("The file named '$sFileName' is not readable.");
		$handle = @fopen($sFileName,'r');
		if (!$handle)
			die("The file named '$sFileName' cannot be opened.");
		$l=1;
		$numCols=0;
		while (($row = fgetcsv($handle, 1000, ",")) !== FALSE) {
			if ($row !== NULL) { // line isn't empty
				for ($i=0; $i<count($row); $i++) {
					$this->sheets[0]['cells'][$l][$i+1] = $row[$i];
				}
