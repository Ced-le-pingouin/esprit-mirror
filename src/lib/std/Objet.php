...
class Objet
{
	function abstraite()
	{
		...
		if ($asAppelant['function'] == $asAppelant['class'] || $asAppelant['function'] == '__construct')
			Erreur::provoquer("Cette classe est abstraite et ne peut donc pas être instanciée");
		else
			Erreur::provoquer("Méthode abstraite non redéfinie");
	}
	
	function implemente($v_sInterface)
	{
		if (!class_exists($v_sInterface))
			Erreur::provoquer("Tentative d'implémenter une interface inexistante: $v_sInterface");
		...
		{
			Erreur::provoquer("Implémentation incomplète de l'interface $v_sInterface. "
