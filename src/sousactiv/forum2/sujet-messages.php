...
foreach ($oSujetForum->aoMessages as $oMessageForum)
{
	...
	$oBloc_Message->remplacer("{personne->nom_complet}",mb_convert_encoding($oMessageForum->oAuteur->retNomComplet(),"HTML-ENTITIES","UTF-8"));
	
	// Email de l'auteur
	if (strlen($sEmail) > 0)
	{
		$oBloc_Message->remplacer("{personne->email}",$oSetEmail);
		$oBloc_Message->remplacer("{a.choix_courriel.href}","?idStatuts=".STATUT_PERS_TUTEUR.(MODALITE_POUR_TOUS == $oForum->retModalite() ? "x".STATUT_PERS_ETUDIANT : "&idEquipes=tous")."&typeCourriel=courriel-forum@".$oForum->retId());
	...
	if ($iNbFichiersAttaches > 0)
	{
		...
		foreach ($oMessageForum->aoRessources as $oRes)
		{
			...
			$sListeFichiersAttaches = str_replace("{a['fichier_attache']->text}",mb_convert_encoding($oRes->retNom()." ({$sTailleFichier})","HTML-ENTITIES","UTF-8"),$sListeFichiersAttaches);
		}
	}
