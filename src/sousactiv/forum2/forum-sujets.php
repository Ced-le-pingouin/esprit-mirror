...
$oTpl->remplacer("{onglet->titre}",mb_convert_encoding($oForum->retNom()." - Liste des sujets","HTML-ENTITIES","UTF-8"));
...
if ($bAfficherListeEquipes)
{
	...
	if ($iNbrEquipes > 0)
	{
		...
		foreach ($oProjet->aoEquipes as $oEquipe)
		{
			...
			$oBlocEquipe->remplacer("{equipe->nom}",mb_convert_encoding($oEquipe->retNom(),"HTML-ENTITIES","UTF-8"));
		}
	}
	else
	{
		...
		$oBlocEquipe->remplacer("{equipe->nom}",mb_convert_encoding("Pas d'équipe trouvée","HTML-ENTITIES","UTF-8"));
	}
