...
require_once(dir_database("hotpotatoes.tbl.php"));
...
class CSousActiv
{
	...
	var $oHotpotatoes;			///< Objet de type CHotpotatoes
	...
	/**
	 * Copie la sous-activité courante vers une activité, en indiquant un n°
	 * d'ordre pour la copie de la sous-activité
	 *
	 * @param	v_iIdDest	l'id de l'activité de destination
	 * @param	v_iNumOrdre	le n° d'ordre de la copie de la sous-activité.
	 * 						Si \c 0 (défaut), elle sera dernière dans l'activité
	 * 						destination
	 *
	 * @return	l'id de la copie de la sous-activité
	 */
...
//				echo "répertoire destination : " . $sRepDst . "<br/>";
	...
	function effacer ()
	{
		switch ($this->retType())
		{
			...
			case LIEN_HOTPOTATOES: $this->effacerHotpotatoes(); break;
	...
	/**
	 * Initialise l'exercice Hotpotatoes
	 */
	function initHotpotatoes() {
		list( , , , $iIdHotpot) = explode(';',$this->oEnregBdd->DonneesSousActiv);
		if (empty($iIdHotpot)) {
			return FALSE;
		} else {
	...
	/**
	 * Efface l'exercice Hotpotatoes
	 */
	function effacerHotpotatoes ()
	{
		$this->initHotpotatoes();
		$this->oHotpotatoes->effacer();
		$this->oHotpotatoes = NULL;
	}
