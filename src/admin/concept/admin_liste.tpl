...
function init()
{
	...
	if (document.getElementById && document.getElementById('idCible'))
	{
		
		var oDomWin  = new DOMWindow(self);
		var oDomElem = new DOMElement("idCible");
...
[BLOCK_FORMATION+]
<div class="formation">
<span id="id_formation"><a{id_cible} class="Formation" href="admin_modif.php?type={formation.type}&params={formation.id}:0:0:0:0:0" target="ADMINFRAMEMODIF" title="{formation.nom}" target="ADMINFRAMEMODIF" onclick="changerMenu({formation.type},{formation.id},0,0,0,0,0); afficherIndice('id_formation'); changerStyle(this);" onfocus="blur()">{formation.nom}</a></span>
[BLOCK_MODULE+]
<div class="module">
<span id="id_module_{module.id}" class="module_intitule">&nbsp;{module.intitule}<a{id_cible} href="admin_modif.php?type=2&params={formation.id}:{module.id}:0:0:0:0" target="ADMINFRAMEMODIF" title="{module.nom}" onclick="changerMenu({module.type},{formation.id},{module.id},0,0,0,0); afficherIndice('id_module_{module.id}'); changerStyle(this);" onfocus="blur()">{module.nom}</a></span>
<hr class="module">
[BLOCK_RUBRIQUE+]
<div class="rubrique">
<span id="id_rubrique_{rubrique.id}">{rubrique.intitule}<a{id_cible} href="admin_modif.php?type={rubrique.type}&params={formation.id}:{module.id}:{rubrique.id}:0:0:0" target="ADMINFRAMEMODIF" onclick="changerMenu({rubrique.type},{formation.id},{module.id},{rubrique.id},0,0,0); afficherIndice('id_rubrique_{rubrique.id}'); changerStyle(this);" onfocus="blur()" title="{rubrique.nom}">{rubrique.nom}</a>&nbsp;&nbsp;<span style="font-size: 7pt; font-weight: normal;">({rubrique.index})</span>
[BLOCK_ACTIVITE+]
<div class="activite">
<span id="id_activ_{activite.id}">{activite.index}.&nbsp;&nbsp;<a{id_cible} href="admin_modif.php?type={activite.type}&params={formation.id}:{module.id}:{rubrique.id}:0:{activite.id}:0" target="ADMINFRAMEMODIF" title="{activite.nom}" onclick="changerMenu({activite.type},{formation.id},{module.id},{rubrique.id},0,{activite.id},0); afficherIndice('id_activ_{activite.id}'); changerStyle(this);" onfocus="blur()">{activite.nom}</a></span>
<div class="sous_activite">
[BLOCK_SOUS_ACTIVITE+]<span id="id_sousactiv_{sousactivite.id}">{sousactivite.index}.&nbsp;&nbsp;<a{id_cible} href="admin_modif.php?type={sousactivite.type}&params={formation.id}:{module.id}:{rubrique.id}:0:{activite.id}:{sousactivite.id}" target="ADMINFRAMEMODIF" title="{sousactivite.nom}" onclick="javascript: changerMenu({sousactivite.type},{formation.id},{module.id},{rubrique.id},0,{activite.id},{sousactivite.id}); afficherIndice('id_sousactiv_{sousactivite.id}'); changerStyle(this);" onfocus="blur()">{sousactivite.nom}</a></span><br>[BLOCK_SOUS_ACTIVITE-]
</div>
</div>
[BLOCK_ACTIVITE-]
</div>
