...
if ($g_iFormation > 0)
{
		...
		foreach ($oProjet->oFormationCourante->aoModules as $oModule)
		{
			...
			if ($oModule->initRubriques() > 0)
			{
				...
				foreach ($oModule->aoRubriques as $oRubrique)
				{
						...
						foreach ($oRubrique->aoActivs as $oActiv)
						{
							...
							if ($oActiv->initSousActivs() > 0)
							{
								...
								foreach ($oActiv->aoSousActivs as $oSousActiv)
								{
									...
									$asReplTpl = array(TYPE_SOUS_ACTIVITE,$iId,$oSousActiv->retNumOrdre(),mb_convert_encoding($sNom,"HTML-ENTITIES","UTF-8"),rawurlencode($sNom),($url_iType == TYPE_SOUS_ACTIVITE && $g_iSousActiv == $iId ? " id=\"idCible\"" : NULL));
							...
							$asReplTpl = array(TYPE_ACTIVITE,$iId,$oActiv->retNumOrdre(),mb_convert_encoding($sNom,"HTML-ENTITIES","UTF-8"),rawurlencode($sNom),($url_iType == TYPE_ACTIVITE && $g_iActiv == $iId ? " id=\"idCible\"" : NULL));
					...
					$asReplTpl = array(TYPE_RUBRIQUE,$iId,mb_convert_encoding($sNom,"HTML-ENTITIES","UTF-8"),rawurlencode($sNom),$sIntitule,$iIdxRubr++,($url_iType == TYPE_RUBRIQUE && $g_iRubrique == $iId ? " id=\"idCible\"" : NULL));
			...
			if ($oModule->retNumDepart() > 0)
				$sIntitule = mb_convert_encoding($oModule->oIntitule->retNom(),"HTML-ENTITIES","UTF-8")
					."&nbsp;"
					.$oModule->retNumDepart()
					."&nbsp;:&nbsp;";
			
			$asReplTpl = array(TYPE_MODULE,$iId,mb_convert_encoding($sNom,"HTML-ENTITIES","UTF-8"),rawurlencode($sNom),$sIntitule,($url_iType == TYPE_MODULE && $g_iModule == $iId ? " id=\"idCible\"" : NULL));
	...
	$asReplTpl = array(TYPE_FORMATION,$g_iFormation,mb_convert_encoding($sNom,"HTML-ENTITIES","UTF-8"),rawurlencode($sNom),($url_iType == TYPE_FORMATION ? " id=\"idCible\"" : NULL));
