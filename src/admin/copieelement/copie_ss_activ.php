...
// Template
// ---------------------
$oTpl = new Template("copie_ss_activ.tpl");
...
$url_iIdActivSrc = (empty($_GET["IdActivSrc"]) ? 0 : $_GET["IdActivSrc"]);
$url_iIdSsActivSrc = (empty($_GET["IdSsActivSrc"]) ? 0 : $_GET["IdSsActivSrc"]);
...
$url_iIdRubDst   = (empty($_GET["IdRubDst"]) ? 0 : $_GET["IdRubDst"]);
$url_iIdActivDst = (empty($_GET["IdActivDst"]) ? 0 : $_GET["IdActivDst"]);
$url_iOrdreSsActivDst   = (empty($_GET["OrdreSsActivDst"]) ? 0 : $_GET["OrdreSsActivDst"]);

if($url_iDstOk==1) // copie de la sous-activité
{
	...
	$oSousActiv = new CSousActiv($oProjet->oBdd,$url_iIdSsActivSrc);
	$iNumOrdreMaxSsActiv = $oSousActiv->retNumOrdreMax($url_iIdActivDst);
	$oSousActiv->defNumOrdre($iNumOrdreMaxSsActiv+1);
	$iIdSousActiv = $oSousActiv->copier($url_iIdActivDst);
	$oSousActiv = new CSousActiv($oProjet->oBdd,$iIdSousActiv);
	$oSousActiv->redistNumsOrdre($url_iOrdreSsActivDst);
...
else
{
	if($url_iSrcOk==1) // Choix de la destination
	{
		...
		$oTpl->remplacer("[IDSSACTIVSRC]",$url_iIdSsActivSrc);
		...
		if($iNbrModules == 0)
		{
			...
			$sOptNumOrdreSsActiv = "<option value='0'>Pas d'activité trouv&eacute;e</option>";
		}
		else
		{
			$sOptionsMod = "";
			...
			else
			{
				...
				{
					// verification que l'activite passé en parametre appartient à la rubrique courante
					if($url_iIdActivDst>0)
					{
						$oActivCourante = new CActiv($oProjet->oBdd,$url_iIdActivDst);
						if($oActivCourante->retIdParent()!=$oRubriqueCourante->retId())
							$url_iIdActivDst=0;
					}
					$sOptionActiv = "";
					for ($i=0; $i<$iNbreActivites; $i++)
					{
						if(($i == 0 && $url_iIdActivDst==0) ||($url_iIdActivDst>0 && $url_iIdActivDst==$oRubriqueCourante->aoActivs[$i]->retId()))
						{
							$sOptionActiv = $sOptionActiv."<option value='".$oRubriqueCourante->aoActivs[$i]->retId()."' selected='selected'>".$oRubriqueCourante->aoActivs[$i]->retNom()."</option>";
							$oActivCourante = new CActiv($oProjet->oBdd,$oRubriqueCourante->aoActivs[$i]->retId());
						}
						else
						{
					...
					// affichage des numeros d'ordre des sous-activités
					$oSousActiv = new CSousActiv($oProjet->oBdd);
					$iNumOrdreMaxSsActiv = $oSousActiv->retNumOrdreMax($oActivCourante->retId());
					$sOptNumOrdreSsActiv = "<option value='1'>1</option>";
					for ($i=2; $i<=$iNumOrdreMaxSsActiv; $i++)
						$sOptNumOrdreSsActiv = $sOptNumOrdreSsActiv."<option value='".$i."'>".$i."</option>";
					if($iNumOrdreMaxSsActiv>=1)
								$sOptNumOrdreSsActiv = $sOptNumOrdreSsActiv."<option value='".($iNumOrdreMaxSsActiv+1)."' selected='selected'>".($iNumOrdreMaxSsActiv+1)."</option>";
				}
			}
		}
		...
		$oTpl->remplacer("[OPTIONSORDRE]",$sOptNumOrdreSsActiv);
	}
	else // choix de la source
	{
		...
		if ($iNbrFormations == 0)
		{
			...
			$sOptionsRub = "<option value='0'>Pas de rubrique trouv&eacute;e</option>";
			$sOptionActiv = "<option value='0'>Pas d'activité trouv&eacute;e</option>";
			$sOptionSsActiv = "<option value='0'>Pas de sous-activité trouv&eacute;e</option>";
		}
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						// verification que l'activite passé en parametre appartient à la rubrique courante
						if($url_iIdActivSrc>0)
						{
							$oActivCourante = new CActiv($oProjet->oBdd,$url_iIdActivSrc);
							if($oActivCourante->retIdParent()!=$oRubriqueCourante->retId())
								$url_iIdActivSrc=0;
						}
						$sOptionActiv = "";
						for ($i=0; $i<$iNbreActivites; $i++)
						{
							if(($i == 0 && $url_iIdActivSrc==0) ||($url_iIdActivSrc>0 && $url_iIdActivSrc==$oRubriqueCourante->aoActivs[$i]->retId()))
							{
								$sOptionActiv = $sOptionActiv."<option value='".$oRubriqueCourante->aoActivs[$i]->retId()."' selected='selected'>".$oRubriqueCourante->aoActivs[$i]->retNom()."</option>";
								$oActivCourante = new CActiv($oProjet->oBdd,$oRubriqueCourante->aoActivs[$i]->retId());
							}
						...
						// affichage de la liste des sous-activités
						$iNbreSsActivites = $oActivCourante->initSousActivs();
						if($iNbreSsActivites==0)
						{
							$sOptionSsActiv = "<option value='0'>Pas de sous-activité trouv&eacute;e</option>";
						}
						else
						{
							$sOptionSsActiv = "";
							for ($i=0; $i<$iNbreSsActivites; $i++)
								$sOptionSsActiv = $sOptionSsActiv."<option value='".$oActivCourante->aoSousActivs[$i]->retId()."'>".$oActivCourante->aoSousActivs[$i]->retNom()."</option>";
						}
					}
				}
			}
		...
		$oTpl->remplacer("[OPTIONSRUB]",$sOptionsRub);
		$oTpl->remplacer("[OPTIONSACTIV]",$sOptionActiv);
		$oTpl->remplacer("[OPTIONSSSACTIV]",$sOptionSsActiv);
	}
}
$oTpl->afficher();
$oProjet->terminer();
