...
// print_r($data->sheets);
	for ($nrow=6; $nrow<$data->sheets[0]['numRows']; $nrow++) {
		...
		/*
		 * Avertissement, mais affectation ‡ une formation
		 */
		$asInfosPers = array(
		"Nom" => $nom
		, "Prenom" => $prenom
		...
		if (preg_match('/importOKPetit/', $res))
		{
			...
			// {{{ R√©cup√©rer les mots de passe des utilisateurs
		...
		/*
		 * On inscrit la personne dans le fichier "mdpncpte" lors de la premiËre inscription
		 * Cel‡ permet de rÈcupÈrer le mot de passe mÍme si la personne ne s'est jamais connectÈe au site.
		 */
		...
		elseif (($sIdFormation!="") && !preg_match('/importErreur/',$res)) {
			...
			// mais pas ‡ ceux qui Ètaient dÈj‡ inscrits ‡ cette formation
