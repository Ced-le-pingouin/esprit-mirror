	...
	foreach ($url_aiIdStatuts as $iIdStatut)
	{
		...
		if (($iNbStatuts = $oMembres->initGraceIdStatut($iIdStatut)) > 0)
		{
			...
			foreach ($oMembres->aoPersonnes as $oPersonne)
			{
				...
				$oBlocMembre->remplacer("{membre.nom}",mb_convert_encoding($oPersonne->retNom(),"HTML-ENTITIES","UTF-8"));
				$oBlocMembre->remplacer("{membre.prenom}",mb_convert_encoding($oPersonne->retPrenom(),"HTML-ENTITIES","UTF-8"));
